_ = '''
        ...
        g_exp.add_argument('--name', type=str, default='', help='name of the experiment. It decides where to store samples and models')
        ...
        g_model.add_argument('--mlp_dim', nargs='+', default=[257, 1024, 512, 256, 128, 1], type=int, help='# of dimensions of mlp. no need to put the first channel')
        g_model.add_argument('--mlp_dim_color', nargs='+', default=[1024, 512, 256, 128, 3], type=int, help='# of dimensions of mlp. no need to put the first channel')
        g_model.add_argument('--mlp_res_layers', nargs='+', default=[2,3,4], type=int, help='leyers that has skip connection. use 0 for no residual pass')
        ...
        parser.add_argument('--num_gen_mesh_test', type=int, default=4, help='how many meshes to generate during testing')
...
for arg in _.split('\n'):
    if 'add_argument(' in arg:
        _arg = arg.split("'--")[1].split("',")[0]
        if 'default=' in arg:
            default = arg.split('default=')[-1].split(",")[0].split(')')[0]
            
        if default != None and default != 'None':
            #print(arg)
            _type = arg.split('type=')[-1].split(",")[0].split(')')[0]
            if 'default=[' in arg:
                default = [ int(num) for num in arg.split('default=')[-1].split("],")[0].replace('[', '').replace(' ', '').split(',') ]
                _type = 'list'
            
            if _type == 'int':
                options[_arg] = int(default)
            elif _type == 'float':
                options[_arg] = float(default)
            elif _type == 'list':
                options[_arg] = list(default)
            else:
                options[_arg] = str(default.replace("'", ""))
        else:
            options[_arg] = None
